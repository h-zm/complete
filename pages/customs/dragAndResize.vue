<template>
    <!-- 新建文件的模板 -->
    <div class="example">
        <contents>DragBlock: 支持4个端点拖动位置，4边拉伸宽高</contents>
        <contents>组件以左上点、右下坐标变化点动态计算浮窗宽高、位置</contents>
        <DragBlock
            :auxiliary="true"
            :callback="handleBack"
            :width="blockWidth"
...
export default {
    ...
    created() {
        let historyInfo = window.localStorage.getItem("dragblock__size");
        if (historyInfo) {
            historyInfo = JSON.parse(historyInfo);
            this.blockHeight = historyInfo.height;
            this.blockWidth = historyInfo.width;
        }
    },
    methods: {
        handleBack(data) {
            console.log("数据变动", data);
            if (data.type == "size") {
