    ...
    <div class="moving-area">
      ...
      >
        <template v-for="(column, sort) in timeList">
          <thead v-if="sort == 0" class="table-head" :key="`head-${sort}`">
            <tr>
            ...
            <tr class="table-tbody-tr">
              ...
              >
                <el-popover
                  ...
                  :open-delay="800"
                  trigger="hover"
                  :content="column.dayName + '：' + time.label"
                >
                  <div
                    class="table-tbody-span"
                    slot="reference"
                    @mousedown.stop="mouseDownItem(time, sort, index, $event)"
                    @mouseup.stop="mouseUpItem(time, sort, index)"
                  ></div>
                </el-popover>
                <!-- {{ time.value }} -->
              </td>
      ...
      <div class="descrip-area" v-for="(time, index) in timeList" :key="index">
...
export default {
  ...
  beforeDestroy() {
    ...
    if (this.useSelecting) {
      this.removeListenerFunc("selecting", "mousedown", this.mousemoveDown);
      this.removeListenerFunc("selecting", "mousemove", this.mousemoveFunc);
      this.removeListenerFunc("selecting", "mouseup", this.mousemoveUp);
      this.removeListenerFunc("selecting", "mouseleave", this.mousemoveLeave);
    }
  },
  methods: {
    getParentInfo() {
      ...
      setTimeout(() => {
        let parentNode = Array.from(document.getElementsByClassName("table"));
        ...
        // 设置可以框选的子节点信息
        let childrenList = document.getElementsByClassName("table-tbody-span");
    ...
    mousemoveFunc(e) {
      ...
      if (this.beginMouse) {
        ...
        this.movingAttribute.width = Math.abs(this.beginPosition.x - e.pageX);
        this.movingAttribute.height = Math.abs(this.beginPosition.y - e.pageY);
      ...
      if (thanOneList.length == 0 && dayObj.indexList.length > 0) {
        ...
        thanOneList.filter((value, index) => {
          if (index == 0) {
            ...
            valueList = dayObj.indexList.slice(indexNumber, value + 1);
          ...
          dayObj.resultList.push({ key: otherDescrip, value: valueList }); // 推入结果列表
          ...
          if (index == thanOneList.length - 1) {
            ...
            dayObj.resultList.push({ key: finalDescrip, value: finalList });
          }
        });
      }
